...
#define HT_PRIME_1 33
#define HT_PRIME_2 47
#define HT_INITIAL_BASE_SIZE 53
#define UPPER_LOAD_LIMIT_PERCENT 70
#define LOWER_LOAD_LIMIT_PERCENT 10
...
void ht_resize(ht_hash_table* ht, const size_t base_size) {
...
void ht_insert(ht_hash_table* ht, const char* key, const char* value) {
    ...
    if (load > UPPER_LOAD_LIMIT_PERCENT) {
        ht_resize_up(ht);
    }
...
void ht_delete(ht_hash_table* ht, const char* key) {
    const size_t load = ht->count * 100 / ht->size;
    if (load < LOWER_LOAD_LIMIT_PERCENT) {
        ht_resize_down(ht);
    }
